...
def response(inputs, history):
    ...
    # === SAFETY FILTER: fånga färgfrågor ===
    if re.search(r"(vilken\s+färg\s+har\s+svampen)|(svampens\s+färg)", user_text.lower()):
        yield (
            "⚠️ Jag kan inte svara direkt på vilken färg svampen har. "
            "Färg kan vara väldigt svår att avgöra på foto och varierar beroende på ljus, ålder och miljö.\n\n"
            "📌 Beskriv gärna svampen själv (hatt, skivor/porer, fot, lukt m.m.) så kan jag hjälpa dig vidare. "
            "Kom ihåg: man ska ALDRIG äta en svamp baserat enbart på en sådan här chatt."
        )
        return
    ...
    if user_files:
        ...
        # === Structured output-anrop för bild ===
        schema = types.Schema(
    ...
    # === Om ingen fråga ställts (endast bild) → generera summering av JSON ===
    if image_part and not user_text.strip() and mushroom_info:
        summary = f"""
• Artförslag: {mushroom_info.get("common_name","?")} (släkte {mushroom_info.get("genus","?")})
• Färg: {mushroom_info.get("color","?")}
• Synliga drag: {", ".join(mushroom_info.get("visible", []))}
• Modellens säkerhet: {mushroom_info.get("confidence",0):.0%}
⚠️ Ät ALDRIG svamp baserat enbart på denna chatt, rådfråga expert/litteratur.
"""
        yield summary.strip()
        return

    # === Annars: vanliga strömmande svaret ===
